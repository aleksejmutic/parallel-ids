...
// Pull the username out of the login endpoint's success body, if present.
function crackedUser(ev) {
  if (ev.source_type === "http" && ev.raw_stdout) {
    try {
      const body = JSON.parse(ev.raw_stdout);
      if (body && body.username) return body.username;
...
function formatLine(ev) {
  ...
  // A "crack" is a SUCCESSFUL auth attempt. request_flood hits /api/health,
  // which always returns 200 — that's success but not a credential crack, so
  // it's excluded or every flood line would look cracked.
  const isCrack = success && attack !== "request_flood";

  const line = document.createElement("div");
  line.className = "line enter " + (isCrack ? "crack" : success ? "ok" : "fail");

  if (isCrack) {
    const who = crackedUser(ev);
    const label = ev.source_type === "ssh" ? "ACCESS GRANTED" : "PASSWORD CRACKED";
    const bits = [who ? `user=${who}` : "", `→ ${dest}`].filter(Boolean).join(" ");
    line.innerHTML =
      `<span class="t">${ts}</span> ` +
      `<span class="crack-badge">✔ ${escapeHtml(label)}</span> ` +
      `<span class="crack-meta">${escapeHtml(bits)}</span>`;
    return line;
  }
